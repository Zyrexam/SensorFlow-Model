...
def create_sliding_windows(df, window_size=200, stride=100):
    ...
    X = np.array(X_windows)  # shape: (num_windows, window_size, 12)
    y = np.array(y_windows).astype(np.int32)  # shape: (num_windows,)
    return X, y


def get_dataset(filepath, window_size=200, stride=100, normalize=True):
    """
    Full dataset pipeline: Load CSV → Window → Normalize → Return X, y
    """
